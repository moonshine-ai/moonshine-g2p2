#!/usr/bin/env python3
"""
Export ``KoichiYasuoka/roberta-small-japanese-char-luw-upos`` to ONNX for :mod:`japanese_tok_pos`.

Requires: torch, onnx, transformers. Default output: ``data/ja/roberta_japanese_char_luw_upos_onnx/``.

Uses ``BertTokenizerFast``-compatible assets (``vocab.txt`` + ``tokenizer_config.json``), same
runtime path as Korean WordPiece in this repo.

Model card: https://huggingface.co/KoichiYasuoka/roberta-small-japanese-char-luw-upos
"""
...
DEFAULT_HF_MODEL = "KoichiYasuoka/roberta-small-japanese-char-luw-upos"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Export Japanese RoBERTa char LUW+UPOS to ONNX.")
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=_repo_root() / "data" / "ja" / "roberta_japanese_char_luw_upos_onnx",
