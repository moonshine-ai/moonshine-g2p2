#!/usr/bin/env python3
"""
Sample ``data/ru/wiki-text.txt`` lines, run :mod:`russian_rule_g2p` per word, and compare
to eSpeak NG (``ru``).

Comparison uses :func:`russian_rule_g2p.coarse_ipa_for_compare` on both outputs so minor
stress placement and eSpeak-specific symbols (e.g. ``ɭ`` vs ``lʲ``) still count as matches
when the coarse phone string agrees.

Usage::

    python scripts/verify_russian_g2p_wiki.py --max-lines 50
    python scripts/verify_russian_g2p_wiki.py --max-lines 100 --stats
"""
...
from russian_rule_g2p import (  # noqa: E402
...
def _wiki_path() -> Path:
    return ROOT / "data" / "ru" / "wiki-text.txt"


def main() -> None:
    p = argparse.ArgumentParser(description="Verify Russian G2P vs eSpeak on wiki-text samples.")
    ...
    if dict_path.is_file():
        lex = load_russian_lexicon(dict_path)
    ...
    for line in wiki.open(encoding="utf-8"):
        ...
        for m in _WORD_RE.finditer(line):
            ...
            if len(key) < 2 or not re.search(r"[а-яё]", key):
                continue
            compared += 1
            if compared > args.max_tokens:
                break

            ours = word_to_ipa(tok, lexicon=lex or None)
            if not ours:
                continue
            try:
